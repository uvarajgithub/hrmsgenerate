...
```bash
git checkout ai/build-001-leave-management || git checkout -b ai/build-001-leave-management
```
...
Implement every assigned Requirement ID (see requirement-scope.json - 288 requirements in scope: 278 platform/UI/application requirements + 10 business workflow requirements).
...
The Build Package itself should already be saved at `.ai-delivery/build-packages/BUILD-001`. Do not
move or rewrite that package unless you are updating delivery evidence.

1. Work only on branch `ai/build-001-leave-management`.
2. Commit all changes.
3. Use commit message: `BUILD-001: Leave Management implementation`
